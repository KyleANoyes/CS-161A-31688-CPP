...
void readScore(string prompt, double& refVar);
...
int main() {
	...
	// Get final and midterm score
	do {
		getInput(midtermScore, finalScore);

		// If not allowed, throw error
		if ((finalScore < 0) || (finalScore > 4)) {
			errorCode(0);
		}
		else if ((midtermScore < 0) || (midtermScore > 4)) {
			errorCode(0);
		}
	} while ((finalScore < 0) || (finalScore > 4) || (midtermScore < 0) || (midtermScore > 4));
	cout << endl;
	...
	// Get final grade
	calcLetterGrade(gradeAverage, finalGradeLetter);
...
double assignAverage(int numAssigns) {
	...
	for (i = 0; i < numAssigns; ++i) {
		do {
			...
			readScore(scorePrompt, newScore);
